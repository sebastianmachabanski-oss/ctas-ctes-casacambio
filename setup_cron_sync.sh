...
echo "⏰ Configurando sincronización automática cada 15 minutos..."

# ══════════════════════════════════════════════════════════════════
# Netlify scheduled function para sync automático
# ══════════════════════════════════════════════════════════════════
mkdir -p netlify/functions

cat > netlify/functions/sync-scheduled.mts << 'EOF'
...
# Actualizar netlify.toml para habilitar funciones scheduled
...
echo ""
echo "IMPORTANTE: Antes del push agregá en Netlify una variable más:"
echo "  SUPABASE_SERVICE_ROLE_KEY = (la service role key de Supabase)"
echo "  Se obtiene en: Supabase → Project Settings → API → service_role"
echo ""
echo "Luego:"
echo "  git add ."
echo "  git commit -m 'feat: sync automatico cada 15 minutos'"
echo "  git push"
